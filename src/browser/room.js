...
function renderMessage(message, options = {}) {
  ...
  // The label a screen reader announces is an attribution surface like any other,
  // and the one where the forgery would land LEAST mitigated (#278): `local copy`,
  // the neutral avatar and the divider are all visual, so a screen-reader user
  // would receive the forged author with none of the honesty signals around it.
  // Read from the guarded map, never from the raw stored `from`.
  replyBtn.setAttribute("aria-label", `Reply to ${displayAuthorOf(message)} #${message.id}`);
...
// The only value that may be DISPLAYED OR ANNOUNCED as a message's author (#278).
// `messagesById` is the chokepoint: `renderMessage` writes this device's own label
// there for a restored record, so every attribution surface inherits the guard
// rather than re-deriving it — which is what four rounds of per-site guarding kept
// missing. The fallback is the local label, never the raw stored alias: a missing
// entry must not be a route back to the unguarded value.
function displayAuthorOf(message) {
  return state.messagesById.get(message.id)?.from ?? RESTORED_SENDER_LABEL;
}
...
function setReply(message) {
  ...
  replyIndicatorLabel.textContent = `Replying to ${displayAuthorOf(message)} #${message.id}`;
  messageText.focus();
}
